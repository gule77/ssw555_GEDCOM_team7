                ...
                for(Family family: familyList) {
                    ...
                    if(husbandId != null && wifeId != null) {
                        if(children!= null && children.contains(husbandId) && children.contains(wifeId)){
                            res =false;
                            System.out.println("ERROR US18: " + family.getLine() + ": "+family.getHusbandID() + " and " + family.getWifeID() + " get married while they are siblings");
                        }
                    }
                }
            }
            ...
            for(Map.Entry<Integer, HashSet<String>> entry: generationMap.entrySet()) {
                if(entry.getKey() >= 3 ) {
                    ...
                    if(sameGen.contains(family.getHusbandID()) && sameGen.contains(family.getWifeID())) {
                        System.out.println("ERROR US19: " + family.getLine() + ": " + family.getHusbandID() + " and " + family.getWifeID() + " married while they are fisrt cousins");
                        res = false;
                    }
                }
            }
